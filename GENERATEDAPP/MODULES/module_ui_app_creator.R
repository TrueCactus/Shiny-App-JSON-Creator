      ...
      lapply(items, function(item) {
        ...
        output[[paste0("dynamic_content_ui_", tab_id)]] <- renderUI({
          ...
          switch(content_type,
                 ...
                 "showdata" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 12, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "showdata-IDX"),
                   textInput(ns(paste0("content_data_", tab_id)), 
                             "Data", value = "data.csv"),
                   textInput(ns(paste0("content_title_", tab_id)), 
                             "Title", value = "data")
                 ),
                 "varfreq" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 6, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "varfreq-IDX"),
                   textInput(ns(paste0("content_data_", tab_id)), 
                             "Data", value = "data.csv")
                 ),
                 ...
                 "crossvartable" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 6, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "crossvar-table-IDX"),
                   textInput(ns(paste0("content_data_", tab_id)), 
                             "Data", value = "data.csv"),
                   textInput(ns(paste0("content_title_", tab_id)), 
                             "Title", value = "CrossVar Table")
                 ),
                 "crossvargraph" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 6, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "crossvar-graph-IDX"),
                   textInput(ns(paste0("content_data_", tab_id)), 
                             "Data", value = "data.csv"),
                   textInput(ns(paste0("content_title_", tab_id)), 
                             "Title", value = "CrossVar Graph")
                 ),
                 "video" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 6, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "video-IDX"),
                   textInput(ns(paste0("content_link_", tab_id)), 
                             "Video Link", 
                             value = "https://www.youtube.com/embed/sQjnvaf072M")
                 ),
                 "image" = tagList(
                   numericInput(ns(paste0("content_width_", tab_id)), 
                                "Width", value = 3, min = 1, max = 12),
                   textInput(ns(paste0("content_id_", tab_id)), 
                             "ID", value = "image-IDX"),
                   textInput(ns(paste0("content_link_", tab_id)), 
                             "Image Link", 
        ...
        observeEvent(input[[paste0("add_content_", tab_id)]], {
          ...
          new_content <- switch(content_type,
                                ...
                                "showdata" = list(
                                  type = "showdata",
                                  width = input[[paste0("content_width_", tab_id)]],
                                  id = input[[paste0("content_id_", tab_id)]],
                                  data = input[[paste0("content_data_", tab_id)]],
                                  title = input[[paste0("content_title_", tab_id)]]
                                ),
                                "varfreq" = list(
                                  type = "varfreq",
                                  width = input[[paste0("content_width_", tab_id)]],
                                ...
                                "video" = list(
                                  ...
                                  width = input[[paste0("content_width_", tab_id)]],
                                  id = input[[paste0("content_id_", tab_id)]],
                                  link = input[[paste0("content_link_", tab_id)]]
                                ),
                                "image" = list(
                                  type = "image",
                                  width = input[[paste0("content_width_", tab_id)]],
